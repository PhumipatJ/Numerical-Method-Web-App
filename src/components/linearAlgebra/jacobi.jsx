...
const Jacobi = () => {
    ...
    const getEquationApi = async () => {
        try {
            const response = await fetch(`https://pj-numer-api.onrender.com/linearAlgebraData/filter?data_id=3&dimension=${Dimension}`);
            if (!response.ok) {
    ...
    const solveAnswer = () => {
        ...
        for (let k = 0; k < maxIterations; k++) {
            ...
            const errors = nextX.map((value, index) => Math.abs(value - currentX[index]) < tolerance ? '0' : (Math.abs(value - currentX[index]) / Math.abs(value) * 100).toFixed(6));
            newSteps.push({
                iteration: k + 1,
