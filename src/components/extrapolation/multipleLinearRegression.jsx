                        ...
                        {[...Array(K)].map((_, xIndex) => (
                            <td key={xIndex}>
                                <Form.Control
                                    type="number"
                                    placeholder={`0`}
                                    value={X[i]?.[xIndex] || 0} // Access the specific x value
...
const solveAnswer = () => {
    ...
    for (let i = 0; i < n; i++) {
        let yi = fx[i]; // Dependent variable (y)
        
        // First row and first column are sums involving the constant (1)
        A[0][0] += 1;  // Count of data points (n)
        B[0] += yi;    // ∑yi

        for (let j = 0; j < K; j++) {
            let xji = X[i][j]; // xji is the j-th independent variable for the i-th data point

            // Fill first row and column (sums involving xji)
            A[0][j + 1] += xji;     // ∑xji
            A[j + 1][0] += xji;     // Symmetric part ∑xji
            B[j + 1] += xji * yi;   // ∑xjiyi

            // Fill the rest of matrix A
            for (let l = 0; l < K; l++) {
                let xli = X[i][l];  // xli is the l-th independent variable for the i-th data point
                A[j + 1][l + 1] += xji * xli;  // ∑xjixli
            }
        }
    }
    console.log("A "+ A);
